...
DUMP_FX    = "--dump-fx" in sys.argv 


def parse_pdhs(data):
    """Return list of (name, blob_bytes) from a PDHS container."""
...
def main():
    ...
    total_ps = total_vs = total_fx = total_skip = 0

    # FX blobs go into a sub-directory so they don't clutter the shader output
    fx_dir = os.path.join(OUTDIR, "fx") if DUMP_FX else None
    if DUMP_FX:
        os.makedirs(fx_dir, exist_ok=True)

    for entry_name, blob in entries:
        safe   = entry_name.replace("/", "_")
        ps_idx = vs_idx = fx_idx = 0
        has_shaders = False
        ...
        for kind, sub in iter_sep_blobs(blob):
            ...
            if magic == PS_MAGIC:
                ...
                # Non-shader blob: FX effect / render-state entry (e.g. CullMode, blend state)
                if DUMP_FX:
                    fname = f"fx_{safe}_{fx_idx}_k{kind}.bin"
                    fpath = os.path.join(fx_dir, fname)
                    open(fpath, "wb").write(sub)
                    print(f"  FX {entry_name}[{fx_idx}] kind={kind:2d} "
                          f"blob={len(sub)} magic={magic.hex()} -> fx/{fname}")
                    fx_idx += 1
                    total_fx += 1
                else:
                    total_skip += 1
    ...
    if DUMP_FX:
        print(f"      {total_fx} FX/render-state blobs written to '{fx_dir}/'")
    else:
        print(f"      {total_skip} non-shader sub-blobs skipped  (re-run with --dump-fx to save them)")
